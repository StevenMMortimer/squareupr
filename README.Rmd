...
One thing to note is that it is important to first pull down the location details for 
your business because this is required when searching for transactions and items to analyze. 
When listing transactions the timeframe defaults to the previous day using `Sys.Date() - 1`.
The `tenders` field lists all methods of payment used to pay in the transaction.
